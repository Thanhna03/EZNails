---
name: clone-website
description: Screenshot an external website, DOWNLOAD its real images + lift its real copy, configure the theme/styling ONCE up front (colors → theme tokens, fonts → roles, reusable utility classes), then visually cut it into ordered sections and scaffold a brand-NEW dedicated component per section via the create-component skill — verified with a screenshot comparison loop. Use whenever the user pastes a website link and asks to clone, replicate, analyze, or "cut" (cắt) a site into sections.
---

# Clone Website Skill

This skill takes an **external website URL**, **screenshots it**, **downloads its real images and lifts its real copy**, configures this template's **theme/styling once up front**, **visually cuts the page into ordered sections**, and scaffolds **one brand-new dedicated component per section** into this Nuxt 3 template using the [create-component](../create-component/SKILL.md) workflow. It **matches the source site's colors and fonts** by updating this template's **theme tokens** (never by hardcoding values in components). To guarantee fidelity it runs a **screenshot comparison loop**: screenshot the source site, render the cloned page locally, compare, and adjust.
...
This skill **delegates** all per-section scaffolding to `create-component` — it does not restate those rules. It only decides *which sections exist (from the screenshot)*, *what each new component is named*, *how to configure the theme*, and *how to bring in the real assets/copy*.
...
1. **Always create a NEW component per section.** For every visually-distinct section, **create a new two-word component** (`category/kebab-name`, Rule 3) via `create-component`. Existing components are **reference only** — never reuse `SectionDefault` / `SectionPhoto` / `section/editor` etc. as the section. There is no "mandatory-reuse" and no "prefer reuse".
...
7. **`setting.json` array fields render the WHOLE array.** Shop info read from `data/setting.json` (`address`, `phone`, `email`, `social`, `iframe_map`, `business.list_item`) MUST be rendered with `v-for` over the entire array + per-item `v-if="item.is_show"`. **Never** grab a single element via `[0]` or `.find(...)`. See create-component's "setting.json array rendering" section.
8. **Every generated section still obeys all CLAUDE.md rules** and passes the full `create-component` checklist. This skill relaxes nothing.
...
### Phase 5 — Create a NEW component per section (delegate to create-component)
For **each** section, run the full [create-component](../create-component/SKILL.md) 3-file workflow (Vue component + content YAML block + CMS `_inputs`). Map captured data:
...
- [ ] Every generated section passes the full `create-component` checklist (3-file rule, Rules 1–11, token-first).
...
| File | Purpose |
|---|---|
| [.claude/skills/create-component/SKILL.md](../create-component/SKILL.md) | Delegated 3-file workflow + checklist (run per section; do not duplicate) |
